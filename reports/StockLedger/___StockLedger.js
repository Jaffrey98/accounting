...
class StockLedger {
    async run(params) {
        ...
        async function organizeEntries(stockEntryData) {
            // let data = [];
            let data = await stockEntryData.map(async function (stockEntry) {
                let dat = await stockEntry.items.map(async function (item) {
                    let d;
            ...
            let dx = await Promise.all(data).then(async function (d) {
                let datax = [];
                console.log('maind');
                console.log(d);
                d.forEach((dd) => {
                ...
                console.log('datax')
                console.log(datax);
                let res = await Promise.all(datax).then(function (datx) {
                    console.log('datx')
                    console.log(datx);
                    return datx;
                });
